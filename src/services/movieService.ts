  ...
  try {
    ...
    if (axios.isAxiosError(error)) {
      ...
      switch (status) {
        case 401:
          throw new Error(
            "Invalid TMDB API key. Please check your Bearer Token in the .env file."
          );
        case 403:
          throw new Error(
            "Access denied. Please check your TMDB API permissions."
          );
        case 404:
        ...
        default:
          if (status && status >= 400) {
            throw new Error(
              `Request failed: ${status} - ${statusText || "Unknown error"}`
            );
          }
      }
